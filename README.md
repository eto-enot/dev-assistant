...
│   │       ├── find_file.py    # Инструмент для поиска файлов по шаблону имени
...
Старницу `chat.html` необходимо скопировать в каталог с рабочим проектом и открыть в браузере. Страница автоматически определяет каталог проекта и передает настройки в сервис ассистента. Адрес подключения к сервису может быть изменен в окне настроек.
