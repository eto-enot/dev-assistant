...
LANGUAGES: dict[str, tree_sitter_language_pack.SupportedLanguage] = {
    "cs": "csharp",
    "ts": "typescript",
    "js": "javascript",
    "py": "python",
...
FUNCTION_BLOCK_NODE_TYPES = ["block", "statement_block", "arrow_expression_clause"]
...
def get_lang_for_file(
    file_name: str,
) -> tree_sitter_language_pack.SupportedLanguage | None:
...
def replace_children(
    ...
    code: bytes,
    blockTypes: List[str],
    collapseTypes: List[str],
    collapseBlockTypes: List[str],
    maxChunkSize: int,
):
    ...
    if block:
        childrenToCollapse = [
            child for child in block.children if child.type in collapseTypes
        ]
        for child in reversed(childrenToCollapse):
            ...
            if grandChild:
                ...
                collapsedChild = code[child.start_byte : start] + replace_collapse(
                    grandChild
                )
...
def class_definition_chunk(node: Node, code: bytes, maxChunkSize: int):
    ...
    isInNamespace = (
        node.parent
        and node.parent.type in ["declaration_list"]
        and node.parent.parent
        and node.parent.parent.type in ["namespace_declaration"]
    )
...
def function_definition_chunk(node: Node, code: bytes, maxChunkSize: int):
    ...
    isInClass = (
        ...
        in ["class_definition", "class_declaration", "impl_item"]
    )
...
def make_text_node(node: Node, content: str, parent: TextNode | None):
    ...
    item = TextNode(
        id_=default_id_func(0, TextNode(text="")),
        text=content,
...
def get_code_path(node: Node | None):
    def _get_name(node: Node):
        name = next(
            (x for x in node.children if x.type in ["identifier", "qualified_name"]),
            None,
        )
        if name:
            assert name.text
        ...
        while parent_node.type not in types + ["namespace_declaration"]:
            ...
            if parent_node.type == "compilation_unit":
                parent_node = next(
                    (
                        x
                        for x in parent_node.children
                        if x.type in ["file_scoped_namespace_declaration"]
                    ),
                    None,
                )
                break
...
class SourceCodeNodeParser(NodeParser):
        ...
        **kwargs: Any,
        ...
        with self.callback_manager.event(
            ...
            for doc in documents_with_progress:
                nodes_from_doc = self._recursively_get_nodes_from_nodes(
                    [doc], 0, show_progress
                )
    ...
    def _recursively_get_nodes_from_nodes(
        ...
        for node in nodes_with_progress:
            ...
            self._default_splitter = SentenceSplitter.from_defaults(
                chunk_size=self.chunk_size, chunk_overlap=self.chunk_size // 2
            )
    ...
    def _code_chunker(self, file_name: str, content: str, max_chunk_size: int):
        ...
        return [
            x
            for x in self._get_chunks(tree.root_node, text_bytes, max_chunk_size, None)
        ]
