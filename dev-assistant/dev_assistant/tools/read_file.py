...
TOOL_DESCRIPTION = """To read a file content with given file name or path, use the read_file tool. \
...
class ReadFileTool(FunctionTool):
    def __init__(self):
        """Read file tool. Used for read a file content"""
        tool_metadata = get_tool_metadata(
            self._read_file, "read_file", TOOL_DESCRIPTION
        )
        super().__init__(self._read_file, tool_metadata)

    async def _read_file(
        self, ctx: Context, path: Annotated[str, "Relative path to file or file name"]
    ):
        logger.debug("ReadFile tool called")
        try:
            work_dir = await ctx.store.get("work_dir", None)
            if not work_dir:
                return (
                    f"I cannot read the {path}, working directory is not set. "
