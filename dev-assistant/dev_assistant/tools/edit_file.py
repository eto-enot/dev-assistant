...
TOOL_DESCRIPTION = """To edit (modify) an existing file use the edit_file tool. This tool performs exact string replacements in files.
...
def normalize_quotes(s: str):
    return s.replace('‘', "'").replace('’', "'").replace('“', '"').replace('”', '"')

def find_normalized_string(content: str, search_string: str):
    if search_string in content:
        return search_string
    
    content = normalize_quotes(content)
    search_string = normalize_quotes(search_string)
    if search_string in content:
        return search_string
    
    return None

def apply_edit(content: str, old_string: str, new_string: str, replace_all: bool):
...
def get_patch(old_content: str, new_content: str):
...
class EditFileTool(FunctionTool):
    ...
    async def _edit_file(
            self, ctx: Context,
            path: Annotated[str, "The relative file name or path to file being edited (modified)"],
        ...
        try:
            ...
            if not work_dir:
                return f"I cannot edit the '{path}', working directory is not set. Please ask user to set working directory."
            print('work dir is:', work_dir)

            if old_string == new_string:
                return f"I cannot edit the '{path}': old_string and new_string are exactly the same. No changes to make."
            ...
            normalized_old = find_normalized_string(content, old_string)
            if not normalized_old:
                return f"I cannot edit the '{path}': string to replace not found in file.\nString: ${old_string}"
            
            idx = content.index(normalized_old)
            try:
                idx = content.index(normalized_old, idx+1)
                return f"I cannot edit the '{path}': found multiple matches of the string to replace, \
                        but replace_all is false. To replace all occurrences, set replace_all to true. To \
                        replace only one occurrence, please provide more context to uniquely identify the \
                        instance.\nString: ${old_string}"
            except ValueError:
                pass

            updated_content = apply_edit(content, normalized_old, new_string, replace_all)
            question = f"Apply the following editings to '{path}'?\n```diff\n{get_patch(content, updated_content)}```"
            ...
            if not response.response:
                return f"User rejected the file modification. File '{path}' was NOT edited. " + \
